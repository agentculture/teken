...
Every failure inside afi raises :class:`AfiError`. The top-level ``main()``
catches it, formats via :mod:`afi.cli._output`, and exits with
...
# Exit-code policy. Documented in ``afi learn`` output and ``docs/rubric.md``.
# 0      = success
...
class AfiError(Exception):
    """Structured error raised within afi; carries a remediation hint for agents."""
