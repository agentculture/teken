...
from teken.cli._output import emit_error

# Note: _commands submodules are imported lazily inside :func:`_build_parser`
# to avoid a circular dependency. teken.cite._engine imports teken.cli._errors;
# eagerly loading teken.cli._commands.cli (which imports teken.cite) at module
# init would create a cycle when teken.cite is the first-touched package.
...
class _AfiArgumentParser(argparse.ArgumentParser):
    ...
    produces no ``hint:`` line, which would make teken itself fail the rubric's
...
def _build_parser() -> argparse.ArgumentParser:
    # Deferred imports (see module-level note): avoids teken.cite ↔ teken.cli cycle.
    from teken.cli._commands import cli as _cli_group
    from teken.cli._commands import doctor as _doctor_cmd
    from teken.cli._commands import explain as _explain_cmd
    from teken.cli._commands import learn as _learn_cmd
    from teken.cli._commands import overview as _overview_cmd
    ...
    # through _AfiArgumentParser too. Without this, `teken cli bogus --foo`
...
def main_afi_alias(argv: list[str] | None = None) -> int:
    """Entry point for the deprecated ``afi`` command.

    The project was renamed ``afi`` → ``teken``. The ``afi`` console script is
