...
``teken cli cite``     — drop the agent-first CLI reference tree into the target
                       project under ``.teken/reference/<lang>-cli/``.
``teken cli doctor``   — run the seven-bundle rubric against a target CLI
                       and surface inconsistencies with actionable remediation
                       (replaces ``teken cli verify``; ``--fix`` applies any
                       auto-fixable handlers).
``teken cli overview`` — read-only descriptive snapshot of a target CLI.

``teken cli verify`` is kept as a deprecated alias that forwards to
...
from teken.cite import SUPPORTED_LANGS, emit_reference
from teken.cli._commands.doctor import cmd_cli_doctor, cmd_cli_verify_deprecated
from teken.cli._output import emit_result
from teken.overview import build as build_overview
...
def cmd_cite(args: argparse.Namespace) -> None:
    """Handler for ``teken cli cite``.

    Returns ``None`` on success (treated as ``EXIT_SUCCESS`` by
    :func:`teken.cli._dispatch`); every failure path raises
    :class:`AfiError`.
    """
    ...
    lines = [
        ...
        (
            "Added `.teken/` to .gitignore"
            if report.gitignore_updated
            else ".gitignore already ignores `.teken/`"
        ),
        "",
        "Next steps:",
    ]
    ...
    lines.extend(
        [
            "",
            "For details on any step:  teken explain cli cite",
            "For the rubric itself:    teken explain cli doctor",
        ]
    )
...
def cmd_cli_overview(args: argparse.Namespace) -> int:
    """Handler for ``teken cli overview [path]``.

    Read-only descriptive snapshot. If ``path`` is missing or points at a
    project without a detectable CLI surface, emits the overview of teken's
...
def register(sub: argparse._SubParsersAction) -> None:
    ...
    cite = cli_sub.add_parser(
        "cite",
        help="Emit the agent-first CLI reference tree into <path>/.teken/reference/.",
    )
    ...
    cite.add_argument(
        "--out",
        default=None,
        help="Override output directory (default: <path>/.teken/reference/<lang>-cli/).",
    )
    ...
    # CI and scripts that call `teken cli verify .` keep working; the deprecation
    # diagnostic is emitted by cmd_cli_verify_deprecated to stderr.
    verify = cli_sub.add_parser(
        "verify",
        help="(Deprecated) Alias for `teken cli doctor`. Removed in v0.6.0.",
    )
    ...
    overview = cli_sub.add_parser(
        "overview",
        help="Read-only descriptive snapshot of the CLI at <path> (defaults: teken template).",
    )
    overview.add_argument(
        ...
        help=(
            ...
            "describe teken's default scaffolded template."
        ),
    )
