"""``teken doctor`` engine — self-diagnosis for an installed teken.

The doctor verb is the agent-first contract for "tell me what's wrong and
how to fix it." For teken itself it surfaces inconsistencies between the
...
``teken cli doctor <path>`` provides via the rubric runner.
...
- :func:`is_healthy` — returns True when no ``error``-severity check
  failed; mirrors the ``teken cli doctor`` non-strict exit-code policy.
"""
...
from teken.doctor._self_checks import run_self_checks
from teken.doctor.fixes import FixOutcome, apply_fix, register_fix
...
class Diagnosis:
    ...
    subject: str  # e.g. "teken self" or a target path
    healthy: bool
...
def is_healthy(results: list[CheckResult]) -> bool:
    ...
    Mirrors the ``teken cli doctor`` non-strict exit-code policy.
    """
...
def run_self_diagnosis() -> Diagnosis:
    ...
    The doctor walks teken's own install: pyproject version vs. package
    ...
    return Diagnosis(subject="teken self", healthy=is_healthy(results), checks=results)
