"""Self-doctor checks for an installed ``teken``.
...
``teken cli doctor <path>``; ``teken doctor`` is the cheap, deterministic
...
the safe fixes for an installed teken (re-installing a corrupt wheel,
...
from teken.cli._commands.learn import _as_json_payload
from teken.explain import resolve as explain_resolve
from teken.rubric import default_rubric
from teken.rubric._types import CheckResult

BUNDLE = "self"

# Files we expect inside an installed teken cite reference tree. Drift here
# usually means a packaging bug (the wheel didn't pull every reference
# file in) — informational, not auto-fixable in-place.
_REFERENCE_ROOT = Path(teken.__file__).resolve().parent / "cite" / "references" / "python-cli"


def _find_repo_root() -> Path | None:
    """Locate the teken source repo by walking up from the package.

    Returns ``None`` when running from an installed wheel (the package is
    inside ``site-packages`` and no ``pyproject.toml`` for ``teken`` is
    findable upward). Self-checks that only matter at dev time degrade to
    ``info`` severity in that case.
    """
    p = Path(teken.__file__).resolve().parent
    while p != p.parent:
...
def _check_surface_coherence_learn() -> CheckResult:
    ...
    return CheckResult(
        ...
        remediation=(
            "add the missing verbs to `_as_json_payload()` in teken/cli/_commands/learn.py "
...
def _check_surface_coherence_explain() -> CheckResult:
    ...
    if not missing:
        return CheckResult(
            ...
            True,
            "info",
            f"every argparse leaf ({len(leaves)} verbs) resolves via teken.explain",
        )
    return CheckResult(
        BUNDLE,
        "surface_coherence_explain",
        ...
        remediation=(
            "add markdown bodies to teken/explain/catalog.py keyed on the verb path "
            "(e.g. ('cli', 'doctor')); without an entry `teken explain <verb>` fails "
...
def _check_reference_tree_present() -> CheckResult:
    ...
    if not manifest.is_file():
        return CheckResult(
            ...
            remediation=(
                "teken's bundled reference tree is incomplete — likely a packaging bug. "
                "Reinstall with `uv tool install --reinstall teken` or file an issue."
            ),
        )
    try:
        ...
        return CheckResult(
            ...
            remediation="reinstall teken or file an issue if the wheel is malformed",
        )
    ...
    if missing:
        return CheckResult(
            ...
            remediation=(
                "the bundled reference tree is incomplete; reinstall teken or report "
    ...
    if bad:
        return CheckResult(
            ...
            remediation=(
                ...
                "callable run(ctx) — see existing bundles under teken/rubric/checks/"
            ),
        )
    return CheckResult(
        BUNDLE,
